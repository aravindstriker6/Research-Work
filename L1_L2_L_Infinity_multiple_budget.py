...
for subsample_size in subsample_sizes:
    ...
    for rep in range(num_reps):
        ...
        for m in range(len(eps)):
            edge_matrixes = {} #Dictionary Created
            for i in range(len(eps[m])):
                new = 0
                for j in range(len(eps[m])):
                    new = D_12 <= float(eps[m][i]) + float(eps[m][j]) # To check whether there is a edge present
                    new = new.astype(float)
                    edge_matrixes[(eps[m][i], eps[m][j])] = new # Saving the edge matrixes in the dictionary
            stack1 = np.hstack((edge_matrixes[(eps[m][0], eps[m][0])], edge_matrixes[(eps[m][0], eps[m][1])])) #horizontal stack
            stack2 = np.hstack((edge_matrixes[(eps[m][1], eps[m][0])], edge_matrixes[(eps[m][1], eps[m][1])]))
            edge_matrix = np.vstack((stack1, stack2)) # Vertically Stacking both the stacks to get the final edge matrix
            num_edges = len(np.where(edge_matrix != 0)[0])
            #print(num_edges, eps[m])
            num_edges_list.append(num_edges)

            n_1 = 2 * subsample_size # The graph created is double the size
            n_2 = 2 * subsample_size
