...
def set_classifier_prob_no_flow(top_level_vertices,sink_idx):
    for item in top_level_vertices:
        if item !=0 and item != sink_idx:
            if item<=n_1:
...
def find_flow_and_split(top_level_indices, graph_rep_array, n_1, n_2, sink_idx):
    ...
    if flow_curr.flow_value == n_1_curr * n_2_curr:
        set_classifier_prob_full_flow(top_level_indices, n_1_curr, n_2_curr, sink_idx)
...
def log_loss(final_classifier_probs,n_1,n_2,n_blocks):
  ...
  for i in range(len(final_classifier_probs)):
        if i<n_1:
            loss1+=np.log(final_classifier_probs[i][0])
        elif i>=n_1:
            loss2+=np.log(final_classifier_probs[i][1])
  loss1=-1*loss1/len(final_classifier_probs)
  loss2 = -1 * loss2/ len(final_classifier_probs)
  return loss1,loss2
